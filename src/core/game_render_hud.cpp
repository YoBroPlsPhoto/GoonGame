...
void Game::RenderHUD() {
      ...
      if (state == GameState::PAUSED) {
        ...
        DrawText(localPlayer.showAdminSettings ? "ADMIN TERMINAL" : "CONTROL TERMINAL",
                 (int)(menuBox.x + 200 - MeasureText(localPlayer.showAdminSettings ? "ADMIN TERMINAL" : "CONTROL TERMINAL", 25) / 2),
                 (int)(menuBox.y + 30), 25,
        ...
        if (localPlayer.showSettings) {
          ...
          // Glock damage as a slider + click-cycle
          ...
          // Range 1–9999, log-ish scale for usability
          ...
          const char* presetLabels[] = {"35 (DEF)", "100", "500", "9999 (1HKO)"};
          float btnW = 65.0f;
          const int presetCount = 4;
          float totalW = presetCount * btnW + (presetCount - 1) * 8.0f;
