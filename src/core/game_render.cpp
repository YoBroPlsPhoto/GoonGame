...
void Game::DrawAdasHazards() {
  for (const auto &stain : adasStains) {
    ...
    DrawCylinder((Vector3){stain.position.x - stain.radius * 0.35f, 0.10f,
                 ...
                 stain.radius * 0.45f, stain.radius * 0.45f, 0.08f, 48, WHITE);
    DrawCylinder((Vector3){stain.position.x + stain.radius * 0.32f, 0.12f,
                           stain.position.z - stain.radius * 0.25f},
                 stain.radius * 0.38f, stain.radius * 0.38f, 0.08f, 48, WHITE);
  }
...
void Game::Render() {
  ...
  Vector3 viewDir = Vector3Normalize(
      Vector3Subtract(localPlayer.camera.target, localPlayer.camera.position));
  ...
  if (state == GameState::GAME || state == GameState::PAUSED) {
    ...
    if (localPlayer.inVehicle && localPlayer.vehicleIndex >= 0) {
      ...
      activeCam.position =
          (Vector3){v->position.x -
                        cosf(vehicleCamOrbitV) * sinf(vehicleCamOrbitH) * dist,
                    v->position.y + sinf(vehicleCamOrbitV) * dist + 8.0f,
                    v->position.z -
                        cosf(vehicleCamOrbitV) * cosf(vehicleCamOrbitH) * dist};
    ...
    for (auto &s : structures) {
      ...
      if (s->type == StructureType::WALL) {
        ...
        // Gun housing / shield
        Color housingCol = {(unsigned char)std::min(255, (int)baseColor.r + 20),
                            (unsigned char)std::min(255, (int)baseColor.g + 15),
                            (unsigned char)std::min(255, (int)baseColor.b + 15),
                            255};
        ...
        DrawCube({-0.35f, 0.5f, 1.5f + 1.5f - recoilOffset}, 0.25f, 0.25f, 3.0f,
                 barrelCol);
        DrawCube({0.35f, 0.5f, 1.5f + 1.5f - recoilOffset}, 0.25f, 0.25f, 3.0f,
                 barrelCol);
    ...
    if (localPlayer.currentWeapon && !localPlayer.inVehicle &&
      ...
      if (isTurretPreview) {
        // Ghost turret
        DrawCube({ghostPos.x, ghostPos.y + 0.3f, ghostPos.z}, 4.0f, 0.6f, 4.0f,
                 Fade(DARKGRAY, ghostPulse * 0.5f));
        DrawCube({ghostPos.x, ghostPos.y + 1.4f, ghostPos.z}, 1.6f, 1.2f, 1.6f,
                 ghostColor);
        DrawCube({ghostPos.x, ghostPos.y + 2.5f, ghostPos.z}, 2.0f, 1.2f, 1.8f,
                 Fade(MAROON, ghostPulse * 0.7f));
        DrawCube({ghostPos.x, ghostPos.y + 2.5f, ghostPos.z + 2.5f}, 0.3f, 0.3f,
                 3.0f, Fade(DARKGRAY, ghostPulse * 0.7f));
        ...
        // Ghost wall
        DrawCube({ghostPos.x, ghostPos.y + 2.0f, ghostPos.z}, 8.0f, 4.0f, 2.5f,
                 ghostColor);
    ...
    if (localPlayer.inVehicle) {
      Vector3 namePos = {localPlayer.position.x, localPlayer.position.y + 4.5f,
    ...
    if (!inCutscene) {
      // --- SNIPER SCOPE OVERLAY & CROSSHAIR ---
      ...
      if (fovToUse < preferredFOV) {
        ...
        if (localPlayer.inVehicle) {
          ...
          if (tank) {
            int sw = GetScreenWidth(), sh = GetScreenHeight(), cx = sw / 2,
                cy = sh / 2;
      ...
      // --- TOP-RIGHT: HUD INFO (Cash, Wave, FPS) ---
      int sw = GetScreenWidth();
      int sh = GetScreenHeight();

      if (showFPS) {
        DrawRectangleRounded({10, 10, 80, 24}, 0.4f, 6, Fade(BLACK, 0.5f));
        DrawText(TextFormat("FPS %d", GetFPS()), 20, 15, 14, LIME);
      }

      DrawRectangleRounded({(float)sw - 200, 10, 190, 60}, 0.2f, 8,
                           Fade(BLACK, 0.6f));
      DrawText(TextFormat("$%d", localPlayer.money), sw - 190, 20, 20, GREEN);
      DrawText(TextFormat("WAVE %d", currentWave), sw - 100, 20, 16, ORANGE);
      DrawText(TextFormat("ENEMIES: %d", (int)enemies.size()), sw - 190, 45, 14,
               LIGHTGRAY);

      // --- BOTTOM-LEFT: PLAYER & VEHICLE HP ---
      ...
      DrawRectangleRounded({10, (float)py, 220, 30}, 0.2f, 8,
                           Fade(BLACK, 0.6f));
      DrawRectangleRounded({15, (float)py + 5, (int)(210 * playerRatio), 20},
                           0.2f, 8, Fade(RED, 0.8f));
      DrawText(
          TextFormat("%d/%d", std::max(0, localPlayer.hp), localPlayer.maxHp),
          20, py + 10, 12, WHITE);
      ...
      if (localPlayer.inVehicle && localPlayer.vehicleIndex >= 0) {
        ...
        float speedRatio =
            std::max(0.0f, std::min(1.0f, fabsf(v->speed) / v->maxSpeed));

        DrawRectangleRounded({10, (float)sh - 40, 220, 30}, 0.2f, 8,
                             Fade(BLACK, 0.6f));
        DrawRectangleRounded({15, (float)sh - 35, (int)(210 * vehRatio), 20},
                             0.2f, 8, Fade(BLUE, 0.8f));
        DrawText(TextFormat("%d/%d", std::max(0, v->health), v->maxHealth), 20,
                 sh - 30, 12, WHITE);

        DrawRectangleRounded({240, (float)sh - 40, 120, 30}, 0.2f, 8,
                             Fade(BLACK, 0.6f));
        DrawRectangleRounded({245, (float)sh - 35, (int)(110 * speedRatio), 20},
                             0.2f, 8, Fade(GREEN, 0.8f));
        DrawText(TextFormat("%d km/h", (int)(fabsf(v->speed) * 100)), 250,
                 sh - 30, 12, WHITE);
      }
      ...
      if (baseRatio > 0) {
        int bx = 10;
        int by = showFPS ? 45 : 10;
        DrawRectangleRounded({(float)bx, (float)by, 250, 30}, 0.2f, 8,
                             Fade(BLACK, 0.6f));
        DrawRectangleRounded(
            {(float)bx + 5, (float)by + 5, (int)(240 * baseRatio), 20}, 0.2f, 8,
            Fade(YELLOW, 0.8f));
        DrawText(TextFormat("NYC BASE: %d/%d", (int)std::max(0.0f, baseHP),
                            (int)maxBaseHP),
                 bx + 10, by + 10, 12, BLACK);
      }
      ...
      if (localPlayer.currentWeapon && localPlayer.currentWeapon->magSize > 0) {
        ...
        DrawRectangleRounded({(float)sw2 - 130, (float)sh2 - 60, 120, 50}, 0.2f,
                             8, Fade(BLACK, 0.6f));
        DrawText(TextFormat("%d / %d", w->currentAmmo, w->magSize), sw2 - 120,
                 sh2 - 55, 24, WHITE);
        DrawText(TextFormat("RES: %d", w->reserveAmmo), sw2 - 120, sh2 - 25, 14,
                 LIGHTGRAY);
        ...
        int sw2 = GetScreenWidth();
        int sh2 = GetScreenHeight();
        DrawRectangleRounded({(float)sw2 - 150, (float)sh2 - 40, 140, 30}, 0.2f,
                             8, Fade(BLACK, 0.6f));
        DrawText("MELEE / ITEM", sw2 - 140, sh2 - 35, 16, LIGHTGRAY);
      }
      ...
      if (!localPlayer.showInventory) {
        ...
        for (int c = 1; c <= 4; c++) {
          ...
          for (size_t i = 0; i < categories[c].size(); i++) {
            ...
            if (strncmp(w->name, "TURRET", 6) == 0 ||
                strncmp(w->name, "WALL", 4) == 0)
              wName = w->name;
    ...
    if (state == GameState::PAUSED) {
      ...
      DrawRectangleLinesEx(menuBox, 2,
                           localPlayer.showAdminSettings ? Fade(ORANGE, 0.9f)
                                                         : GOLD);
      const char *termTitle =
          localPlayer.showAdminSettings ? "ADMIN TERMINAL" : "CONTROL TERMINAL";
      DrawText(termTitle,
      ...
      if (localPlayer.showSettings) {
        ...
        Rectangle fpsBtn = {menuBox.x + 50, menuY, 300, 40};
        bool fpsHov = CheckCollisionPointRec(GetMousePosition(), fpsBtn);
        DrawRectangleRounded(fpsBtn, 0.2f, 8,
                             fpsHov ? Fade(DARKGRAY, 0.8f) : Fade(BLACK, 0.5f));
        DrawText(TextFormat("SHOW FPS: %s", showFPS ? "ON" : "OFF"),
                 fpsBtn.x + 20, fpsBtn.y + 10, 18, WHITE);
        if (fpsHov && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
          showFPS = !showFPS;
        }
        menuY += 60;
        ...
        if (menuBtn(localPlayer.isFlying ? "FLY MODE: ON" : "FLY MODE: OFF",
                    flyCol)) {
        ...
        DrawText(TextFormat("%d", Glock::globalGlockDamage), menuBox.x + 270,
                 menuY - 5, 22, ORANGE);
        menuY += 28;
        ...
        DrawRectangleRounded(
            {dmgBar.x, dmgBar.y, dmgBar.width * dmgT, dmgBar.height}, 0.5f, 8,
            ORANGE);
        ...
        for (int p = 0; p < presetCount; p++) {
          ...
          DrawRectangleRounded(
              pr, 0.3f, 8,
              active ? Fade(ORANGE, 0.9f)
                     : (hov2 ? Fade(GRAY, 0.8f) : Fade(BLACK, 0.5f)));
          DrawText(
              presetLabels[p],
              (int)(pr.x + pr.width / 2 - MeasureText(presetLabels[p], 13) / 2),
  ...
  if ((state == GameState::GAME || state == GameState::PAUSED) && inCutscene) {
  ...
  if (state == GameState::LOBBY) {
    ...
    if (localPlayer.isAdmin) {
      ...
      DrawRectangleRounded((Rectangle){drawCity.x + 3, drawCity.y + 3,
                                       drawCity.width - 6, drawCity.height - 6},
                           0.1f, 8, cityBase);

      DrawText("NEW YORK CITY", (int)drawCity.x + 20, (int)drawCity.y + 20, 25,
               WHITE);
    ...
    int py = 240;
    DrawRectangleRounded((Rectangle){(float)sw - 460, (float)py - 10, 360, 50},
                         0.1f, 8, Fade(BLACK, 0.6f));
    DrawText(TextFormat("%s - %s (ID: %d)", localPlayer.isAdmin ? "CMD" : "YOU",
    ...
    if (localPlayer.isAdmin) {
      ...
      DrawRectangleRounded(drawBtn, 0.3f, 8, hov ? WHITE : Fade(MAROON, 0.8f));
  ...
  if (state == GameState::MENU) {
    ...
    if (menu.shouldStartHost) {
      ...
      if (!started)
        started = net.StartServerAutoPort(1234, 10, menu.hostName);
      if (started) {
    ...
    if (menu.shouldStartJoin) {
      net.SetLocalPlayerName(menu.playerNick);
      if (net.StartClient(menu.joinIP,
                          menu.joinPort > 0 ? menu.joinPort : 1234)) {
    ...
    if (menu.currentState == MenuState::JOIN) {
      ...
      if (net.discoveredServers.empty()) {
        ...
        DrawText(emptyText, GetScreenWidth() / 2 - emptyW / 2, listY + 15, 16,
                 LIGHTGRAY);
      }
      for (auto const &[ip, info] : net.discoveredServers) {
        ...
        const char *tag =
            info.relayRoomId != 0 ? "RELAY" : (info.internet ? "NET" : "LAN");
        Color bgCol = info.relayRoomId != 0
                          ? DARKPURPLE
                          : (info.internet ? DARKPURPLE : DARKBLUE);
        DrawRectangleRounded(rec, 0.2f, 8, Fade(hov ? SKYBLUE : bgCol, 0.8f));
        DrawText(TextFormat("%s [%s] %d/%d", info.name.c_str(), tag,
