...
Vector3 Tank::GetBarrelTip() const {
  ...
  Vector3 pivotWorld = {position.x + sinf(totalRot) * 4.2f, position.y + 4.0f,
                        position.z + cosf(totalRot) * 4.2f};

  return {pivotWorld.x + forward.x * barrelLen,
...
Vector3 Tank::GetMGTip() const {
  ...
  return {position.x + rx, position.y + 5.8f,
          position.z + rz + forward.z * 2.5f};
}

BoundingBox Tank::GetBoundingBox() {
  return {{position.x - 7.0f, 0, position.z - 9.0f},
...
void Tank::Draw() {
  ...
  DrawCylinder({1.5f, 5.1f, 1.0f}, 0.8f, 0.8f, 0.2f, 12,
               hullColor); // Loader hatch
  ...
  // Tapered Barrel
  DrawCylinderEx({0, 0, 0.5f}, {0, 0, 12.0f}, 0.55f, 0.4f, 20,
                 {30, 30, 30, 255});
